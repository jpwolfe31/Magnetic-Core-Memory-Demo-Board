...
case 1: // write/read
  ...
  strcpy(oled_line6_str, " 6 7 - 100K "); // 7 endCnt added later
...
void executeAction() 
{
if (relayState == 0) { // almost all actions require the relay to be on
  relayOn();
  } 
switch (menuState) {
case 1: // write/read
...
if (key == '7'){ // write random numbers 1M
  ...
  endCnt = maxCnt;
  startCnt = 0;
  if (endCnt == 500000) {startCnt = 32;} // does not evenly divid by 64
  strcpy(oled_title_str, "    write/read    ");
  randomWriteInhRead(startCnt, endCnt, 0, strEndCnt, oled_title_str);  // no inhibit
  }
}
...
if (key == '7'){ // write random numbers 1M +
  ...
  randomWriteInhRead(startCnt, endCnt, 1, strEndCnt, oled_title_str);  
  }
}
...
while(1){  
  ...
  writeCnt = writeCnt + 64;   
  if (writeCnt > writeCntMax) {  
    //if (writeCntMax > 6400){// not used - this keeps the relay on when not needed 
    //  while(1){ // hold screen, wait for key to continue
    //    relayOff(); 
    //    updateLeds();
    //    if (getKey()){break;}
    //    }
