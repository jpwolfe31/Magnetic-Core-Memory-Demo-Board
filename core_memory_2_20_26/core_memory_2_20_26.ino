...
// Check thermistor every 2 seconds
if (cm - pmTherm >= 2000){ 
...
// Check voltages every 2 seconds - for testing
if (cm - pmVoltage >= 2000){
...
if (cm - pmDisplayActive >= 500) { 
  ...
  if ( ((cm - lmMotion) <= (1 * 60000)) // time in minutes for motion - 1
    || ((cm - lmCommand) <= (1 * 60000)) ){ // time in minutes for command - 1
    relayOn();
...
if (blueLedState == 1) {return;} // blue supercedes
